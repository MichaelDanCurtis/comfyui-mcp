...
import { ChatGptOAuthBackend, CHATGPT_DEFAULT_MODEL, CHATGPT_SYSTEM_PROMPT } from "./chatgpt-oauth-backend.js";
...
LORA MANAGER & VAULT — before applying a LoRA, consult the curated catalog so you use the right trigger keywords, base-model pairing, and strength. Call lora_catalog_sync after new downloads. If willmiao ComfyUI LoRA Manager is installed (lora_catalog_detect_lora_manager), run lora_catalog_import_sidecars to pull Civitai trigger words, usage_tips, and previews from .metadata.json sidecars; otherwise lora_catalog_enrich_civitai hashes files against CivitAI. Then lora_catalog_list / lora_catalog_get / lora_catalog_search. For training data: use PhotoMapAI (photomap_curate_sync or vault_create_training_pack) to pick a diverse subset from an album, then vault_list_training_pack_images + runcomfy_trainer_* to upload and train. Let the user browse LoRAs with panel_open_lora_manager or panel_pick_lora. Persist learned usage with lora_catalog_upsert; attach previews with lora_catalog_set_preview. Wire LoraLoader using rel_path and recommended strength — include trigger keywords in the prompt.
...
export async function runPanelOrchestrator(): Promise<void> {
  ...
  // Register every editable prompt's built-in default so the /prompts editor can
  ...
  registerPrompt("backend.chatgpt", "ChatGPT (OAuth) base prompt", CHATGPT_SYSTEM_PROMPT, "Applies on the next ChatGPT chat/session.");
