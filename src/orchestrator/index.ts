...
LORA MANAGER — before applying a LoRA, consult the curated catalog so you use the right trigger keywords, base-model pairing, and strength. Call lora_catalog_sync after new downloads, then lora_catalog_list / lora_catalog_get / lora_catalog_search for metadata (setup_instructions, keywords, strength_default). Let the user browse or pick interactively with panel_open_lora_manager or panel_pick_lora (previews + descriptions when the panel UI supports it). When you learn how a LoRA should be used, persist it with lora_catalog_upsert; attach a sample image with lora_catalog_set_preview so future picks are recognizable. Wire LoraLoader using the catalog's rel_path and recommended strength — include trigger keywords in the prompt.
